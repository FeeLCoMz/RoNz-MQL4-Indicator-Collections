...
enum ENUM_ENTRYSIGNLEVEL
  {
   NoEntry=0,MA1=1,MA2=2,MA3=3,MA4=4,RSI=5
  };
...
extern string MAPeriod="5,10,20,30";   // MAPeriods (x,x,x,x)
extern int    RSIPeriod=3;//RSI Period (RSI EntrySignLevel Only)
extern int    RSILevel=30;//RSI Level
extern int    InpShift=0;  // Shift
extern ENUM_ENTRYSIGNLEVEL    EntrySignLevel=MA1;    //Entry Sign Level
extern ENUM_EXITSIGNLEVEL    ExitSignLevel=StopLoss_Exit;    //Exit Sign Level
extern bool   InpUseRSI=false;//Use RSI for Entry Sign
extern int    inpStopLoss=0;//Stop Loss
extern int    inpTakeProfit=0;//Take Profit
extern bool   UseCloseTime=false; //Use Close Time to Exit Order
extern bool   InpShowProfitLabel=false;      // Show profit label on entry sign
...
int OnInit()
  {
   ...
   StringSplit(MAPeriod,StringGetCharacter(",",0),sMAPeriod);
   ...
   MyShortName=MyShortName+" ("+(string)sMAPeriod[0]+","+(string)sMAPeriod[1]+","+(string)sMAPeriod[2]+","+(string)sMAPeriod[3]+")";
...
//---- 
   ...
   SetIndexLabel(0,"MA1 "+(string)sMAPeriod[0]);
   SetIndexShift(0,InpShift);
   SetIndexDrawBegin(0,InpShift+(int)sMAPeriod[0]);

   SetIndexBuffer(1,ExtMA2);
   SetIndexLabel(1,"MA2 "+(string)sMAPeriod[1]);
   SetIndexShift(1,InpShift);
   SetIndexDrawBegin(1,InpShift+(int)sMAPeriod[1]);

   SetIndexBuffer(2,ExtMA3);
   SetIndexLabel(2,"MA3 "+(string)sMAPeriod[2]);
   SetIndexShift(2,InpShift);
   SetIndexDrawBegin(2,InpShift+(int)sMAPeriod[2]);

   SetIndexBuffer(3,ExtMA4);
   SetIndexLabel(3,"MA4 "+(string)sMAPeriod[3]);
   SetIndexShift(3,InpShift);
   SetIndexDrawBegin(3,InpShift+(int)sMAPeriod[3]);
   ...
   for(int i=0; i<limit && !IsStopped(); i++)
     {
      ...
      ExtMA1[i] = iMA(NULL, 0, (int)sMAPeriod[0], 0, inpMethod, inpPrice, i);
      ExtMA2[i] = iMA(NULL, 0, (int)sMAPeriod[1], 0, inpMethod, inpPrice, i);
      ExtMA3[i] = iMA(NULL, 0, (int)sMAPeriod[2], 0, inpMethod, inpPrice, i);
      ExtMA4[i] = iMA(NULL, 0, (int)sMAPeriod[3], 0, inpMethod, inpPrice, i);
     }
...
//RSI Entry Only
   if(InpUseRSI && EntrySignLevel==5)
     {
      //RSI Overbuy/Oversell Settings
      supportUp=pRSI<=(RSILevel) && RSI>RSILevel;// && low>pLow
      resistDown=pRSI>=(100-RSILevel) && RSI<(100-RSILevel);//&& high<pHigh 
                                                            //RSI Break Setting
   ...
   bool priceUp=close>ExtMA1[x];
   bool priceDown=close<ExtMA1[x];

   bool priceCrossMA1Up=close>ExtMA1[x] && open<ExtMA1[x];
   bool priceCrossMA1Down=close<ExtMA1[x] && open>ExtMA1[x];
   bool priceCrossMA2Up=close>ExtMA2[x] && open<ExtMA2[x];
   bool priceCrossMA2Down=close<ExtMA2[x] && open>ExtMA2[x];
   bool priceCrossMA3Up=close>ExtMA3[x] && open<ExtMA3[x];
   bool priceCrossMA3Down=close<ExtMA3[x] && open>ExtMA3[x];
   bool priceCrossMA4Up=close>ExtMA4[x] && open<ExtMA3[x];
   bool priceCrossMA4Down=close<ExtMA3[x] && open>ExtMA3[x];

   bool ma1Up=ExtMA1[x]>ExtMA2[x];
   bool ma1Down=ExtMA1[x]<ExtMA2[x];
   bool ma2Up=ExtMA2[x]>ExtMA3[x];
   bool ma2Down=ExtMA2[x]<ExtMA3[x];
   bool ma3Up=ExtMA3[x]>ExtMA4[x];
   bool ma3Down=ExtMA3[x]<ExtMA4[x];
   ...
   switch(EntrySignLevel)
     {
      case 1://MA1
         ShortBuy=priceCrossMA1Up && !OverSell;
         ShortSell=priceCrossMA1Down && !OverSell;
         break;
      case 2://MA2
         ShortBuy=ma1Up && supportUp && !OverBought;
         ShortSell=ma1Down && resistDown && !OverSell;
         break;
      case 3://MA3
         ShortBuy=ma2Up && supportUp && !OverBought && ma1Up;
         ShortSell=ma2Down && resistDown && !OverSell && ma1Down;
         break;
      case 4://MA4
         ShortBuy=ma3Up && supportUp && !OverBought  && ma1Up;
         ShortSell=ma3Down && resistDown && !OverSell  && ma1Down;
         break;
      case 5://RSI
         ShortBuy=(supportUp || resistUp);
         ShortSell=(resistDown || supportDown);
         break;
      default:
         ShortBuy=false;
         ShortSell=false;
...
//---- Show entry sign and profit label
   ...
   CreateMTFMASign(NULL,MAPeriod,0,iYPos+11,inpMethod,inpPrice,0,MyID,true,false);
