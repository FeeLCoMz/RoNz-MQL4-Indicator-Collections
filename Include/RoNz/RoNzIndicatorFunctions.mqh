...
double CalculateMultiIndicator(
                               ...
                               )
  {
   ...
   int RSIPeriod=lMAPeriod[1];

   ArrayResize(mTrend,ArraySize(lMAPeriod)+InitialIndicatorsCount);
   ...
   mTrend[6]="STO";//+(string)lSTOCHPeriod;
   mTrend[7]= "RSI "+(string)RSIPeriod;
   ...
   for(i=0; i<ArraySize(mTrend);i++)

     {
      ...
      if(i==ArraySize(lMAPeriod)+3)
        {
         MySign[i].value=iRSI(cSymbol,cTimeframe,RSIPeriod,linputPrice,shift);
         MySign[i].value=NormalizeDouble(MySign[i].value,2);
...
//+------------------------------------------------------------------+
//SignChar : 108 Circle, 110 Rectangle
void CreatePairGroupSignSymbol(const int iSubWindow,string lblName,color clr,int xDist,int yDist,const uchar SignChar=110,const int SignSize=8)
  {
   ObjectCreate(lblName,OBJ_LABEL,iSubWindow,0,0);
   ObjectSetText(lblName,CharToStr(SignChar),SignSize,"Wingdings",clr);
   ObjectSet(lblName,OBJPROP_CORNER,Corner);
...
//|                                                                  |
//+------------------------------------------------------------------+
void CheckAvailableSymbols(string sSymbols="")
  {
   int j=0;
   ...
   symPair[0] = StringSubstr(sSymbols,0,3);
   symPair[1] = StringSubstr(sSymbols,3,3);
      ...
      if(SymbolAvailable)
        {
         if(sSymbols!="")
           {
            if((StringFind(sRequiredSymbols[i],symPair[0])!=-1) || (StringFind(sRequiredSymbols[i],symPair[1])!=-1))
              {
               j++;
